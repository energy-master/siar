...
def test_every_line_of_a_frame_is_the_same_width(size, columns, lines):
    ...
    # Wide terminals are capped rather than stretched: a bar three hundred columns long is harder
    # to read than one a third of that, not easier.
    assert widths.pop() == min(columns, 120)


@pytest.mark.parametrize("lines", [10, 14, 24, 40])
def test_a_frame_never_outgrows_the_terminal(size, lines):
    size(100, lines)
...
def test_the_bar_and_the_totals_survive_a_terminal_too_short_for_the_rest(size):
    ...
    frame = display._render(time.time())
    assert "0/12 files" not in frame[1], "the bar should have moved"
    assert "realtime" in frame[2]
...
def test_a_wide_frame_puts_stages_and_structures_side_by_side(size):
    ...
    frame = "\n".join(display._render(time.time()))
...
def test_stages_are_listed_worst_first(size):
    size(100, 40)
    display = _busy(TuiDisplay("all_structures"))
    stages = [line.split()[0] for line in display._stage_lines()]
