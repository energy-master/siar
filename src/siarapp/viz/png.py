...
This package writes exactly one kind of image — an 8-bit RGB raster, no transparency, no
interlacing, no palette. That is about forty lines of PNG, and writing them here means a
customer's ``pip install`` does not drag in Pillow (or, worse, matplotlib) to save a 5 KB
thumbnail.

The format: an 8-byte signature, then length-prefixed, CRC-suffixed chunks. ``IHDR``
(dimensions and colour type), ``IDAT`` (zlib-compressed scanlines, each prefixed with a filter
byte), ``IEND``. Filter type 0 ("None") on every scanline — spectrograms are noisy, so the
predictive filters buy almost nothing and cost clarity.

Ported from ``siar/src/siar/viz/png.py``; the two should stay identical.
"""
